...
(function (factory) {
    ...
    var dom = function () {
         ...
         * @param {Boolean} [options.isSkipPaddingBlankHTML] - theme: false
         * @param {Boolean} [options.isNotSplitEdgePoint] - theme: false
    ...
    var dialog = renderer.create('<div class="modal" aria-hidden="false" tabindex="-1"/>', function ($node, options) {
        ...
        $node.html(['<div class="modal-dialog">', '  <div class="modal-content">', options.title ? '    <div class="modal-header">' + '      <h4 class="modal-title">' + options.title + '</h4>' + '      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>' + '    </div>' : '', '    <div class="modal-body">' + options.body + '</div>', options.footer ? '    <div class="modal-footer">' + options.footer + '</div>' : '', '  </div>', '</div>'].join(''));
    });
    ...
    var checkbox = renderer.create('<label class="custom-control custom-checkbox"></label>', function ($node, options) {
        ...
        $node.html([' <input type="checkbox" class="custom-control-input"' + (options.id ? ' id="' + options.id + '"' : ''), (options.checked ? ' checked' : '') + '/>', ' <span class="custom-control-indicator"></span>', ' <span class="custom-control-description">' + (options.text ? options.text : '') + '</span>', '</label>'].join(''));
    });
    ...
    var Style = function Style() {
         ...
         * passing an array of style properties to .scss()
         ...
         * @param {String} [options.nodeName] - theme: `SPAN`
         * @param {Boolean} [options.expandClosestSibling] - theme: `false`
         * @param {Boolean} [options.onlyPartialContains] - theme: `false`
         * @return {Node[]}
         */
    ...
    var VideoDialog = function VideoDialog(context) {
        ...
        this.initialize = function () {
            ...
            var body = '<div class="form-group note-form-group row-fluid">' + '<label class="note-form-label">' + lang.video.url + ' <small class="text-muted">' + lang.video.providers + '</small></label>' + '<input class="note-video-url form-control  note-form-control note-input span12" ' + ' type="text" />' + '</div>';
