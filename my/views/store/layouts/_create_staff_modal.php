<?php
    /* @var $this yii\web\View */
    /* @var $store \common\models\stores\Stores */
    /* @var $model \my\models\forms\CreateStoreStaffForm */

    use my\models\forms\CreateStoreStaffForm;
    use common\models\stores\StoreAdmins;
    use my\components\ActiveForm;
    use yii\bootstrap\Html;
    use yii\helpers\Url;

    $model = new CreateStoreStaffForm();
?>
<div class="modal fade" id="createStaffModal" tabindex="-1" data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title"><?= Yii::t('app', 'stores.create_staff.header')?></h4>
            </div>

            <?php $form = ActiveForm::begin([
                'id' => 'createStaffForm',
                'action' => Url::toRoute('/stores/staff/create/' . $store->id),
                'fieldConfig' => [
                    'template' => "{label}{input}",
                ],
                'options' => [
                    'class' => 'form'
                ]
            ]); ?>
                <div class="modal-body">
                    <?= $form->errorSummary($model, [
                        'id' => 'createStaffError'
                    ]); ?>

                    <?= $form->field($model, 'account') ?>

                    <div class="form-group">
                        <label for=""><?= $model->getAttributeLabel('password') ?></label>
                        <div class="input-group">
                            <?= Html::textInput('CreateStaffForm[password]', '', ['class' => 'form-control password'])?>
                            <span class="input-group-btn random-password">
                                <button class="btn btn-default" type="button" id="staff_edit_gen"><i class="fa fa-random fa-fw" data-toggle="tooltip" data-placement="right" title="Generate password"></i></button>
                            </span>
                        </div>
                    </div>

                    <?= $form->field($model, 'status')->dropDownList(StoreAdmins::getStatuses()) ?>

                    <div class="form-group">
                        <label for="">Access</label><br>
                        <?php foreach ($model->getAccessRules() as $code => $label) : ?>
                            <?php if ('providers' == $code) continue; ?>
                            <label class="checkbox-inline">
                                <?= Html::checkbox('CreateStaffForm[access][' . $code . ']', true, ['class' => 'access'])?>
                                <?= $label ?>
                            </label>
                        <?php endforeach; ?>
                    </div>

                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal"><?= Yii::t('app', 'stores.create_staff.modal_cancel')?></button>
                    <?= Html::submitButton(Yii::t('app', 'stores.create_staff.modal_submit'), [
                        'class' => 'btn btn-outline btn-primary',
                        'name' => 'create-staff-button',
                        'id' => 'createStaffButton'
                    ]) ?>
                </div>
            <?php ActiveForm::end(); ?>
        </div>
    </div>
</div>