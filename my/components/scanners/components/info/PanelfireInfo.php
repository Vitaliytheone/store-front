 ...
 */
class PanelfireInfo extends BasePanelInfo
{
    /**
    ...
    public function checkStatusActive()
    {
        ...
        return
            (
                boolval(stripos($content, 'class="form-control" name="user_name"')) &&
                boolval(strpos($content, 'class="form-control" name="user_password"'))
            )
            ||
            (
                boolval(strpos($content, 'type="text" required="" name="username"')) &&
