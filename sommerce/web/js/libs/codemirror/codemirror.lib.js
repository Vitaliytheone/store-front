...
(function (mod) {
    ...
    function CodeMirror(place, options) {
        ...
        if (options.autofocus && !mobile || cm.hasFocus()) setTimeout(bind(onFocus, this), 20);else onBlur(this);
    ...
    function initScrollbars(cm) {
        ...
        cm.display.scrollbars = new CodeMirror.scrollbarModel[cm.options.scrollbarStyle](function (node) {
            ...
            if (axis == "horizontal") setScrollLeft(cm, pos);else setScrollTop(cm, pos);
        }, cm);
    ...
    function updateLineForChanges(cm, lineView, lineN, dims) {
        for (var j = 0; j < lineView.changes.length; j++) {
            ...
            if (type == "text") updateLineText(cm, lineView);else if (type == "gutter") updateLineGutter(cm, lineView, lineN, dims);else if (type == "class") updateLineClasses(lineView);else if (type == "widget") updateLineWidgets(cm, lineView, dims);
        }
    ...
    function handlePaste(e, cm) {
        ...
        if (pasted) {
            e.preventDefault();
            if (!cm.isReadOnly() && !cm.options.disableInput) runInOp(cm, function () {
    ...
    TextareaInput.prototype = copyObj({
        ...
        poll: function poll() {
            ...
            runInOp(cm, function () {
                ...
                if (text.length > 1000 || text.indexOf("\n") > -1) input.value = self.prevInput = "";else self.prevInput = text;
        ...
        onContextMenu: function onContextMenu(e) {
            ...
            function rehide() {
                ...
                if (te.selectionStart != null) {
                    ...
                    var i = 0,
                        poll = function poll() {
                        if (display.selForContextMenu == cm.doc.sel && te.selectionStart == 0 && te.selectionEnd > 0 && input.prevInput == "\u200B") operation(cm, commands.selectAll)(cm);else if (i++ < 10) display.detectingSelectAll = setTimeout(poll, 500);else display.input.reset();
                    };
    ...
    ContentEditableInput.prototype = copyObj({
        init: function init(display) {
            ...
            on(div, "compositionend", function (e) {
                ...
                if (!ours) return;
                if (e.data != ours.startData && !/\u200b/.test(e.data)) ours.data = e.data;
            ...
            on(div, "input", function () {
                ...
                if (cm.isReadOnly() || !input.pollContent()) runInOp(input.cm, function () {
                    regChange(cm);
                });
            });
        ...
        startGracePeriod: function startGracePeriod() {
            ...
            this.gracePeriod = setTimeout(function () {
                ...
                if (input.selectionChanged()) input.cm.operation(function () {
        ...
        receivedFocus: function receivedFocus() {
            var input = this;
            if (this.selectionInEditor()) this.pollSelection();else runInOp(this.cm, function () {
                input.cm.curOp.selectionChanged = true;
            });
        ...
        applyComposition: function applyComposition(composing) {
            if (this.cm.isReadOnly()) operation(this.cm, regChange)(this.cm);else if (composing.data && composing.data != composing.startData) operation(this.cm, applyTextInput)(this.cm, composing.data, 0, composing.sel);
        },
    ...
    function movePos(doc, pos, dir, line) {
        if (dir < 0 && pos.ch == 0) {
            if (pos.line > doc.first) return _clipPos(doc, Pos(pos.line - 1));else return null;
    ...
    function mapFromLineView(lineView, line, lineN) {
        ...
        for (var i = 0; i < lineView.rest.length; i++) {
            ...
            if (lineNo(lineView.rest[i]) > lineN) return { map: lineView.measure.maps[i], cache: lineView.measure.caches[i], before: true };
        }
    }
    ...
    function onMouseDown(e) {
        ...
        switch (e_button(e)) {
            case 1:
                ...
                if (cm.state.selectingText) cm.state.selectingText(e);else if (start) leftButtonDown(cm, e, start);else if (e_target(e) == display.scroller) e_preventDefault(e);
                break;
            case 2:
    ...
    function leftButtonDown(cm, e, start) {
        ...
        var sel = cm.doc.sel,
            ...
            contained;
        if (cm.options.dragDrop && dragAndDrop && !cm.isReadOnly() && type == "single" && (contained = sel.contains(start)) > -1 && (cmp((contained = sel.ranges[contained]).from(), start) < 0 || start.xRel > 0) && (cmp(contained.to(), start) > 0 || start.xRel < 0)) leftButtonStartDrag(cm, e, start, modifier);else leftButtonSelect(cm, e, start, type, modifier);
    }
    ...
    function leftButtonStartDrag(cm, e, start, modifier) {
        ...
        var dragEnd = operation(cm, function (e2) {
            ...
            if (Math.abs(e.clientX - e2.clientX) + Math.abs(e.clientY - e2.clientY) < 10) {
                ...
                if (webkit || ie && ie_version == 9) setTimeout(function () {
        ...
        function extendTo(pos) {
            ...
            if (type == "rect") {
                ...
                var anchor = oldRange.anchor,
                    head = pos;
                if (type != "single") {
                    if (type == "double") var range = cm.findWordAt(pos);else var range = new Range(Pos(pos.line, 0), _clipPos(doc, Pos(pos.line + 1, 0)));
    ...
    function onDrop(e) {
        ...
        if (files && files.length && window.FileReader && window.File) {
            ...
            var loadFile = function loadFile(file, i) {
                if (cm.options.allowDropFileTypes && indexOf(cm.options.allowDropFileTypes, file.type) == -1) return;
            ...
            try {
                ...
                if (text) {
                    ...
                    if (selected) for (var i = 0; i < selected.length; ++i) {
                        _replaceRange(cm.doc, "", selected[i].anchor, selected[i].head, "drag");
    ...
    var wheelEventDelta = function wheelEventDelta(e) {
    ...
    CodeMirror.prototype = {
        ...
        cursorCoords: function cursorCoords(start, mode) {
            ...
            if (start == null) pos = range.head;else if ((typeof start === "undefined" ? "undefined" : _typeof(start)) == "object") pos = _clipPos(this.doc, start);else pos = start ? range.from() : range.to();
        ...
        heightAtLine: function heightAtLine(line, mode) {
            ...
            return intoCoordSystem(this, lineObj, { top: 0, left: 0 }, mode || "page").top + (end ? this.doc.height - _heightAtLine(lineObj) : 0);
        },
        ...
        moveH: methodOp(function (dir, unit) {
            ...
            cm.extendSelectionsBy(function (range) {
                if (cm.display.shift || cm.doc.extend || range.empty()) return _findPosH(cm.doc, range.head, dir, unit, cm.options.rtlMoveVisually);else return dir < 0 ? range.from() : range.to();
            }, sel_move);
        }),
        ...
        moveV: methodOp(function (dir, unit) {
            ...
            doc.extendSelectionsBy(function (range) {
                ...
                var headPos = _cursorCoords(cm, range.head, "div");
                if (range.goalColumn != null) headPos.left = range.goalColumn;
                goals.push(headPos.left);
                var pos = _findPosV(cm, headPos, dir, unit);
                if (unit == "page" && range == doc.sel.primary()) addToScrollPos(cm, null, _charCoords(cm, pos, "div").top - headPos.top);
                return pos;
            }, sel_move);
            ...
            cm.doc.iter(lineNo, cm.display.viewTo, function (line) {
                if (line.widgets) for (var i = 0; i < line.widgets.length; i++) {
                    if (line.widgets[i].noHScroll) {
                        regLineChange(cm, lineNo, "widget");break;
                    }
                }++lineNo;
            });
            cm.curOp.forceUpdate = true;
    ...
    var Init = CodeMirror.Init = { toString: function toString() {
                ...
                for (var i = 0; i < ranges.length; i++) {
                    ...
                    if (line) {
                        ...
                        if (cur.ch > 0) {
                            ...
                            if (prev) cm.replaceRange(line.charAt(0) + cm.doc.lineSeparator() + prev.charAt(prev.length - 1), Pos(cur.line - 1, prev.length - 1), Pos(cur.line, 1), "+transpose");
                        }
                    }
    ...
    function _markText(doc, from, to, options, type) {
        ...
        if (cm) {
            ...
            if (marker.collapsed) regChange(cm, from.line, to.line + 1);else if (marker.className || marker.title || marker.startStyle || marker.endStyle || marker.css) for (var i = from.line; i <= to.line; i++) {
    ...
    function stretchSpansOverChange(doc, change) {
        ...
        if (!sameLine) {
            ...
            if (gap > 0 && first) for (var i = 0; i < first.length; ++i) {
                if (first[i].to == null) (gapMarkers || (gapMarkers = [])).push(new MarkedSpan(first[i].marker, null, null));
            }for (var i = 0; i < gap; ++i) {
    ...
    function adjustScrollWhenAboveVisible(cm, line, diff) {
        if (_heightAtLine(line) < (cm.curOp && cm.curOp.scrollTop || cm.doc.scrollTop)) addToScrollPos(cm, null, diff);
    }
    ...
    function getHandlers(emitter, type, copy) {
        ...
        if (copy) return arr && arr.length > 0 ? arr.slice() : noHandlers;else return arr || noHandlers;
    }
    ...
    function signalDOMEvent(cm, e, override) {
        if (typeof e == "string") e = { type: e, preventDefault: function preventDefault() {
    ...
    // handling this'.
    var Pass = CodeMirror.Pass = { toString: function toString() {
    ...
    if (document.createRange) range = function range(node, start, end, endNode) {
        ...
        return r;
    };else range = function range(node, start, end) {
    ...
    if (ie && ie_version < 11) activeElt = function activeElt() {
        try {
    ...
    var bidiOrdering = function () {
        ...
        function charType(code) {
            if (code <= 0xf7) return lowTypes.charAt(code);else if (0x590 <= code && code <= 0x5f4) return "R";else if (0x600 <= code && code <= 0x6ed) return arabicTypes.charAt(code - 0x600);else if (0x6ee <= code && code <= 0x8ac) return "r";else if (0x2000 <= code && code <= 0x200b) return "w";else if (code == 0x200c) return "b";else return "L";
        }
