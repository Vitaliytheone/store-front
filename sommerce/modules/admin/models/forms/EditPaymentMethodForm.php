...
class EditPaymentMethodForm extends StorePaymentMethods
{
    ...
    public function getMethodFormData()
    {
        ...
        foreach ($settingForm as $key => $settings) {
            $result[$key]['label'] = Html::label(Yii::t('admin', $settings['label']), 'editpaymentmethod-' . $settings['code']);
            $result[$key]['parentClass'] = 'form-group';

            switch ($settings['type']) {
                case 'input':
                    $result[$key]['content'] = Html::input(
                        'text', $settings['name'],
                ...
                case 'checkbox':
                    $result[$key]['label'] = Yii::t('admin', $settings['label']);
                    $result[$key]['additionalElement'] = true;
                    $result[$key]['parentClass'] = 'form-check';
                    $result[$key]['content'] = Html::checkbox(
                        $settings['name'],
                        $options[$settings['code']],
                        [
                            'id' => 'editpaymentmethod-' . $settings['code'],
                ...
                case 'multi_input':
                    // TODO add required form element
                    break;

                case 'select':
                    $result[$key]['content'] = Html::dropDownList(
                        $settings['name'],
                        $options[$settings['code']],
                        $settings['options'],
                        [
                ...
                case 'textarea':
                    $result[$key]['content'] = Html::textarea(
                        $settings['name'],
                        $options[$settings['code']],
                        [
                            'id' => 'editpaymentmethod-' . $settings['code'],
