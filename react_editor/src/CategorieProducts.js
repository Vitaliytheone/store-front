...
class CategorieProducts extends Component {
  ...
  getPackage = productIndex => packageIndex => async () => {
    const getPackageId = this.state.data[productIndex].packages[packageIndex]
	  .id;
	  console.log(getPackageId);
	const response = await get_update_package(getPackageId);
	console.log(response.data.provider_service);
    const responseServices = await get_providers_services(
      response.data.provider_id
	);
	console.log(responseServices.data);
	const a = find(responseServices.data, (o) => o.service == response.data.provider_service );
	console.log(a);
	const b = responseServices.data.filter((item) => item.service !== response.data.provider_service)
	console.log(b);
    ...
    this.setState({
      response: {
		...
		services: {
			providerServices: responseServices.data
		}
      }
    });
  };
