...
class AdditionalServices extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['panel_id', 'type', 'status', 'search', 'start_count', 'refill', 'cancel', 'auto_services', 'auto_order', 'processing', 'show_id', 'input_type', 'string_type', 'string_name', 'provider_service_id_label', 'store'], 'integer'],
            [['content'], 'string'],
            [['date'], 'safe'],
            [['name'], 'string', 'max' => 32],
    ...
    public function beforeSave($insert)
    {
        if ($insert) {
            ...
            $maxRes = static::find()->max('panel_id');
            $this->panel_id = (int)$maxRes + 1;
            $this->provider_id = $this->panel_id;
        }
    ...
    {
        return $this->hasMany(UserServices::class, ['provider_id' => 'id']);
    }
